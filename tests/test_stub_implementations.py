...
def test_kismet_netxml_export(tmp_path: Path) -> None:
    ...
    manager._handshakes = {
        "a": HandshakeInfo(
            id="a", bssid="46:04:ba:73:4d:4e", essid="Test<&>Net",
            capture_path="", hash_path="", hashcat_mode=22000,
            gps_lat=1.5, gps_lon=-2.5,
        )
    }
...
async def test_slsa_verification_fails_closed_without_verifier() -> None:
    ...
    verifier_cls = next(
        obj for obj in vars(security).values()
...
def test_wpa3_transition_detection_and_plugin_wiring() -> None:
    ...
    assert WPA3DowngradeAttack.is_transition_mode({"encryption": "WPA3", "pmf": "required"}) is False
    assert WPA3DowngradeAttack.is_transition_mode({"encryption": "WPA2", "pmf": "disabled"}) is False
    ...
    import inspect
    handler_src = inspect.getsource(WiFiPlugin.__module__ and __import__("urban_hs.modules.wifi.plugin", fromlist=["x"]))
