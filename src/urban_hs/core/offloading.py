...
class HashtopolisClient:
    ...
    async def upload_hash_file(
        self, hash_file: Path, task_name: str = "urban-hs-auto"
    ) -> HashtopolisTask | None:
        ...
        if not self.api_token:
            logger.info(
                "Hashtopolis API token not configured, skipping remote upload", path=str(hash_file)
            )
            return None
        ...
        try:
            ...
            if res_data.get("response") == "OK":
                ...
                logger.info(
                    "Successfully uploaded hash file to Hashtopolis", hashlist_id=hashlist_id
                )
                return HashtopolisTask(task_id=hashlist_id, name=task_name, status="QUEUED")
        except Exception as exc:
            logger.warning(
                "Failed to upload hash file to Hashtopolis", path=str(hash_file), error=str(exc)
            )
